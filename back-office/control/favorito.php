<?php
      class favorito extends simplePHP {
        ...
        public function __construct() {
            ...
            $this->keys['activefavorito'] = 'active';
        ...
        public function _actionListar() {
          ...
          #dados do cliente
            ...
            $this->keys['select_cliente'] = $this->html->select(false, $cliente, 'cliente_id',$_SESSION['filtros']['favorito']['cliente_id'],0);

            #dados do estabelecimento
            $estabelecimento = $this->model->getList("estabelecimento","id","nome");
            $this->keys['select_estabelecimento'] = $this->html->select(false, $estabelecimento, 'estabelecimento_id',$_SESSION['filtros']['favorito']['estabelecimento_id'],0);
          ...
          $total = $this->model->countData('favorito',$_SESSION['filtros']['favorito']);
          ...
          $dados = $this->model->getData('favorito','*',$_SESSION['filtros']['favorito'],$limits);

          if($_SESSION['filtros']['favorito'] != '') {
            $this->keys['limpar'] = '<a href="/favorito/limpafiltros" class="btn btn-info"><i class="glyphicon glyphicon-zoom-out" aria-hidden="true"></i> Limpar</a>';
             ...
             $tabela[0]['favorito'] = 'Favorito';
              ...
              foreach($dados as $dado) {
                ...
                $tabela[$x]['cliente'] = $cliente[$dado['cliente_id']];
                 ...
                 $tabela[$x]['favorito'] = $dado['favorito'];
                  
                $tabela[$x]['acoes'] = $this->html->link('Ver',"/favorito/ver/$dado[id]",'','btn btn-info btn-xs');

                $x++;
              }
              $this->keys['tabela'] = $this->html->table($tabela,array('class'=>'table table-bordered table-condensed table-hover table-striped upper tabela-listar ','id'=>'lista-favorito'),true,'','',true);
          } else {
              $this->keys['tabela'] = $this->html->div('Não foram encontrados favorito cadastrados  ',array('class'=>'center'));
          }
        ...
        public function _actionGrava() {
          ...
          $this->model->addData('favorito',$_POST,true);
          die('sucesso;');
        }
        ...
        public function _actionAltera() {
          ...
          $dado_id = $_REQUEST['id'];
          $this->model->alterData('favorito',$_POST,array('id' => $dado_id));
          die('sucesso;');
        }
        ...
        public function _actionVer() {
          ...
          $this->keys += $this->model->getOne('favorito',$dado_id);
